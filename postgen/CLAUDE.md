...
### 投稿形式：2段階ツリー（2026-08-25・本人指示で標準化）

**明日以降、10投稿すべてをこの形式で作る。** テストプレイで確認済み。

- 生成した1本を2ツイートに分割する。区切り方は、本文中のどこか
  「、」で終わる位置を選び、そこで1ツイート目を切る（句点「。」ではなく読点「、」で切る）
- 切る位置の目安：話の核心（原因の判明・意外な発見・落ち）の直前で切ると、
  続きが気になって2ツイート目に進んでもらいやすい
- フォーマットは既存の「■Nツイート目」形式をそのまま使う：
  ```
  [ポスト本文]
  ■1ツイート目
  （1ツイート目の本文。最後は「、」で終わる）
  ...
  ■2ツイート目
  （2ツイート目の本文。CTAなどの締めもここに入れる）
  ==========
  ```
- `convert_tsv.py` は「■Nツイート目」を検出すると自動でツリー扱いにする
  （同じA列番号・同じ投稿時刻で2行に分かれる）。追加の変更は不要
- 全ジャンル（施術エピソード・トラウマ・上下関係・知識型等）に適用する。
  短い投稿（知識型の一言など）でも、自然な「、」の切れ目を探して2分割する
